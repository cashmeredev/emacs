...
(defun my/agent-shell-ret-binding ()
  (let ((keymap (get-text-property (point) 'keymap)))
    (when keymap
      (let ((binding (lookup-key keymap (kbd "RET"))))
        (when (commandp binding)
          binding)))))

(defun my/agent-shell-output-at-point-p ()
  (or (get-text-property (point) 'agent-shell-ui-state)
      (and (> (point) (point-min))
           (get-text-property (1- (point)) 'agent-shell-ui-state))))

(defun my/agent-shell-ret ()
  (interactive)
  (if-let* ((binding (my/agent-shell-ret-binding)))
      (call-interactively binding)
    (cond
     ((my/agent-shell-output-at-point-p)
...
(use-package fossil-ui
  ...
  (fossil-ui-allow-configured-binary-commits t)
  ...
  (with-eval-after-load 'zoom
    (add-to-list 'zoom-ignored-major-modes 'fossil-ui-mode))
  (with-eval-after-load 'golden-ratio
    (add-to-list 'golden-ratio-exclude-modes 'fossil-ui-mode)))
...
(keymap-set my/leader-open-map "s" #'sync-ui)
...
(keymap-set my/leader-code-map "t" #'justl)
...
(keymap-set my/local-leader-map "d" #'minimal-dashboard)
...
(with-eval-after-load 'fossil-ui
  (evil-define-key '(normal motion visual) fossil-ui-mode-map
    ...
    (kbd "]c") #'fossil-ui-next-hunk
    (kbd "[c") #'fossil-ui-previous-hunk
    (kbd "j") #'evil-next-line
    (kbd "k") #'evil-previous-line
    (kbd "n") #'fossil-ui-next-file
    (kbd "p") #'fossil-ui-previous-file
...
(with-eval-after-load 'justl
  (evil-set-initial-state 'justl-mode 'normal)
  (evil-set-initial-state 'justl-module-mode 'normal)
  (evil-set-initial-state 'justl-compile-mode 'normal)
  (evil-define-key 'normal justl-mode-map
    (kbd "q") #'quit-window
    (kbd "r") #'justl--refresh-buffer
    (kbd "e") #'justl-exec-recipe
    (kbd "E") #'justl--exec-recipe-with-args
    (kbd "m") #'justl--show-modules
    (kbd "RET") #'justl-go-to-recipe
    (kbd "?") #'justl-help-popup)
  (evil-define-key 'normal justl-module-mode-map
    (kbd "q") #'quit-window
    (kbd "r") #'justl--module-refresh-buffer
    (kbd "e") #'justl-exec-module
    (kbd "o") #'justl--module-open-justl
    (kbd "RET") #'justl--go-to-module
    (kbd "?") #'justl-module-help-popup)
  (evil-define-key 'normal justl-compile-mode-map
    (kbd "q") #'quit-window
    (kbd "r") #'justl-recompile))
