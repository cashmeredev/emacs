...
(with-eval-after-load 'org
  ...
  (setq org-todo-keywords
        '((sequence "TODO(t)" "NEXT(n)" "ACTIVE(a!)" "WAIT(w@/!)" "PLAN(p)" "|" "DONE(d!)" "CANCELLED(c@)")))
...
(with-eval-after-load 'org
  ;; Update effort values for longer focus sessions (night owl, deep work)
  ;; Added 4h, 8h, 16h, 40h for multi-hour deep work blocks
  (setq org-global-properties
        '(("EFFORT_ALL" . "0:30 1:00 2:00 4:00 8:00 16:00 40:00")
          ("COMPLEXITY_ALL" . "low medium high")
          ("PRIORITY_ALL" . "A B C D")))
  
  ;; Enhanced column view format with clocksum comparison
...
(defun my/create-project ()
  ...
  (let* ((project-name (read-string "Project name: "))
    ...
    (insert "** Phases\n\n")
    (insert "*** PLAN Phase 1: Planning & Research\n")
    (when (and deadline-str (not (string-empty-p deadline-str)))
      (insert "DEADLINE: <" deadline-str ">\n"))
    (insert ":PROPERTIES:\n")
    ...
    (insert "*** PLAN Phase 2: Implementation\n")
    (insert ":PROPERTIES:\n")
    (insert ":CREATED: " (format-time-string "[%Y-%m-%d %a %H:%M]") "\n")
    (insert ":END:\n\n")
    
    (insert "*** PLAN Phase 3: Review & Polish\n")
    (insert ":PROPERTIES:\n")
    (insert ":CREATED: " (format-time-string "[%Y-%m-%d %a %H:%M]") "\n")
    (insert ":END:\n\n")
...
(defun my/daily-review ()
  "Append a simple daily review to today's journal - whenever you feel like it."
  (interactive)
  (require 'denote)
  ...
  (let* ((today (format-time-string "%Y-%m-%d"))
    ...
    ;; Go to end and add review section
    (goto-char (point-max))
    (unless (save-excursion
              (beginning-of-line)
              (looking-at "^\\*\\* Day Closed"))
      (insert "\n** Day Closed " (format-time-string "[%H:%M]") "\n\n"))
    
    (insert "*** What I did today\n")
    (insert "#+BEGIN: clocktable :scope agenda :maxlevel 2 :block today\n")
    (insert "#+END:\n\n")
    
    (insert "*** Brain dump\n")
    (insert "Thoughts, notes, captures, whatever:\n\n")
    
    (insert "*** Projects check\n")
    (insert "Quick status (or skip if nothing changed):\n")
    (insert "- \n\n")
    
    (insert "*** Tomorrow\n")
    (insert "What to focus on (1-3 things):\n")
    (insert "- \n\n")
    ...
    (org-mode)
    (goto-char (point-max))
    (search-backward "*** Brain dump" nil t)
    (forward-line 2)
    (end-of-line)))

(defun my/reflection ()
  "Create a reflection note - use whenever you feel like reviewing progress."
  (interactive)
  (require 'denote)
  (let* ((title (read-string "Reflection on: " "Recent progress"))
         (file-name (denote title '("reflection"))))
    (find-file file-name)
    (goto-char (point-max))
    (insert "\n* " title "\n\n")
    
    (insert "** Time spent recently\n\n")
    (insert "#+BEGIN: clocktable :scope agenda :maxlevel 2 :block thisweek\n")
    (insert "#+END:\n\n")
    (insert "Or change to :block lastweek, :block thismonth, etc.\n\n")
    
    (insert "** What's working?\n")
    (insert "- \n\n")
    
    (insert "** What's not working?\n")
    (insert "- \n\n")
    
    (insert "** Projects status\n")
    (insert "#+BEGIN: columnview :id global :match \"project\" :format \"%ITEM %PROGRESS %EFFORT\"\n")
    (insert "#+END:\n\n")
    
    (insert "** What needs attention?\n")
    (insert "- \n\n")
    
    (insert "** Ideas / Changes\n")
    (insert "- \n")
    
    (org-mode)
    (goto-char (point-min))
    (search-forward "** What's working?" nil t)
    (forward-line 1)
    (end-of-line)))

(defun my/org-capture-timeblock-to-journal ()
  "Capture a time block to today's journal (refile later if needed)."
  (require 'denote)
  (require 'denote-journal)
  (let* ((description (read-string "What: "))
         (datetime (org-read-date t nil nil "When (date + time): "))
         (duration (completing-read "Duration: "
                                    '("30m" "1h" "1h30m" "2h" "3h" "4h" "6h" "8h")
                                    nil nil "2h"))
         (type (completing-read "Type: "
                               '("Work" "Personal" "Meal" "Break" "Focus" "Learning" "Exercise")
                               nil nil "Work"))
    ...
    ;; Go to end and add time block
    (goto-char (point-max))
    (insert "\n* TODO " description "\n")
    (insert "SCHEDULED: <" datetime ">\n")
    (insert ":PROPERTIES:\n")
    (insert ":DURATION: " duration "\n")
    (insert ":TYPE: " type "\n")
    (insert ":CREATED: " (format-time-string "[%Y-%m-%d %a %H:%M]") "\n")
    (insert ":END:\n\n")
    (insert "Capture in journal - refile with C-c C-w if needed\n")
    (current-buffer)))

(defun my/org-capture-recurring-to-journal ()
  "Capture a recurring time block to today's journal (habits like meals, exercise)."
  (require 'denote)
  (require 'denote-journal)
  (let* ((description (read-string "What (recurring): "))
         (start-date (org-read-date nil nil nil "Start date: "))
         (time (read-string "Time (HH:MM): " "12:00"))
         (repeat (completing-read "Repeat: "
                                 '(".+1d" ".+2d" ".+1w" ".+1m")
                                 nil nil ".+1d"))
         (duration (completing-read "Duration: "
                                   '("15m" "30m" "1h" "1h30m" "2h")
    ...
    ;; Go to end and add recurring block
    (goto-char (point-max))
    (insert "\n* TODO " description "\n")
    (insert "SCHEDULED: <" start-date " " time " " repeat ">\n")
    (insert ":PROPERTIES:\n")
    (insert ":STYLE: habit\n")
    (insert ":DURATION: " duration "\n")
    (insert ":TYPE: " type "\n")
    (insert ":CREATED: " (format-time-string "[%Y-%m-%d %a %H:%M]") "\n")
    (insert ":END:\n\n")
    (insert "Recurring habit - tracked with org-habit. Refile with C-c C-w if needed.\n")
    (current-buffer)))

(defun my/org-capture-meeting-to-journal ()
  "Capture a meeting to today's journal (can refile to separate file later if needed)."
  (require 'denote)
  (require 'denote-journal)
  (let* ((topic (read-string "Meeting topic: "))
         ...
         (duration (completing-read "Duration: "
                                   '("15m" "30m" "1h" "1h30m" "2h")
                                   nil nil "1h"))
         (participants (read-string "Participants (optional): "))
         (today (format-time-string "%Y-%m-%d"))
         (journal-files (denote-directory-files "journal"))
         (today-journal (seq-find
                        (lambda (file)
    ...
    (insert ":CREATED: " (format-time-string "[%Y-%m-%d %a %H:%M]") "\n")
    (insert ":END:\n\n")
    (when (and participants (not (string-empty-p participants)))
      (insert "** Attendees\n")
      (insert "- " participants "\n\n"))
    (insert "** Agenda\n")
    (insert "- \n\n")
    ...
    (insert "** Action Items\n")
    (insert "- [ ] \n\n")
    (insert "Captured in journal - refile to separate meeting file with C-c C-w if needed\n")
    (org-mode)
    (search-backward "** Agenda" nil t)
    (forward-line 1)
    (end-of-line)
    (current-buffer)))

;; Add to org-capture-templates (captures to journal, refile later if needed)
     ...
     (
       (org-ql-block '(and (scheduled :to today)
         ...
         ((org-ql-block-header "📅 Today's Schedule")
          (org-super-agenda-groups
           '(
             (:name "🌙 Night (00:00-06:00)"
              :and (:scheduled today :time-grid t)
              :pred (lambda (item)
       ...
       (org-ql-block '(and (or (tags "self")
         ...
         ((org-ql-block-header "📋 Unscheduled Tasks")
          (org-super-agenda-groups
           '(
             (:name "🔥 ACTIVE (Currently Working)"
              :todo "ACTIVE"
              :order 1)
             (:name "⏭️ NEXT (Ready to Start)"
              :todo "NEXT"
              :order 2)
             (:name "📝 TODO (Backlog)"
              :todo "TODO"
...
;; org-global-properties now defined in workflow enhancements section above
...
;; org-todo-keywords now defined in workflow enhancements section above
...
;; org-set-effort-from-complexity now defined in workflow enhancements section above
...
(use-package vertico-posframe
  ...
  (setq vertico-posframe-width        124                       ;; Narrow frame
...
(use-package maple-modeline
  ...
  :config
  (setq maple-modeline-separator 'arrow)
...
(set-face-attribute 'default nil :family "MonoLisa Nerd Font" :weight 'regular :height cashmere/font-height)
...
(my-leader
  ...
  "dl" '(denote-link t :wk "Link note")
  "dg" '(consult-denote-grep t :wk "Search")
  "dn" '(denote t :wk "Create a new note")
  "dr" '(denote-rename-file t :wk "Rename Note")
  "dp" '(my/create-project :wk "Create project")
  "dR" '(my/daily-review :wk "Daily review")
  "dD" '(my/reflection :wk "Reflection")
  ...
  "o" '(:ignore t :wk "org/open")
  "oa" '(:ignore t :wk "agenda")
  "oad" '(org-agenda :wk "daily dashboard")
  "oaw" '((lambda () (interactive) (org-agenda nil "w")) :wk "weekly overview")
  "oap" '((lambda () (interactive) (org-agenda nil "p")) :wk "projects")
  "oae" '((lambda () (interactive) (org-agenda nil "e")) :wk "evaluation")
  "oac" '((lambda () (interactive) (org-agenda nil "c")) :wk "complexity matrix")
