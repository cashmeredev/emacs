...
(use-package flycheck
  ...
  ;; Org >= 9.8 returns the report line as a propertized string (with an
  ;; `org-lint-marker' property), while flycheck's bundled `org-lint' checker
  ;; expects a number and chokes with "Wrong type argument:
  ;; number-or-marker-p".  Redefine the checker with a coercing parser.
