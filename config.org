...
(setq cashmere/frame-alpha 100
      cashmere/frame-alpha-background nil
      cashmere/font-family "SFMono Nerd Font"
      cashmere/font-height 180
      cashmere/theme 'tinty)
#+end_example
...
The lock file is activated in =init.el= before the first package order is
declared.  It is deliberately refreshed only through an explicit command, so
normal package updates cannot silently rewrite pinned revisions.
...
Load the local, opinionated Org formatter from =lisp/org-fmt.el= on demand. Formatting stays explicit because the formatter can refuse unsafe input; use =SPC c f= (or =SPC m f= in an Org buffer) to format the current buffer.
...
Use =org-modern= only for quiet structural markers in Org files. GTD labels, timestamps, priorities, and the agenda keep their existing semantic faces.
...
All buffer UIs share the v1.4 pixel-aware layout engine.  Pixel mode keeps
tables, flex rows, boxes, buttons, and fields aligned when Nerd Font glyphs,
emoji, CJK, scaled text, or variable-pitch faces are mixed.
...
** company-abort-and-normal-state
...
Built-in tree-sitter mode (Emacs 29+). The =c= grammar ships via =treesit-grammars.with-all-grammars= in =emacs.nix=, so no on-the-fly compilation is needed.
...
** projectile-dashboard-fullscreen
...
Privileged ZFS operations stay non-interactive by default. Unlocking runs authentication, =cryptsetup=, ZFS label detection and pool import as one interactive =doas= operation, because OpenDoas binds its =persist= timestamp to the process session and Emacs creates a fresh session for each external command. Copland's PAM SSH-agent authentication asks for the Nitrokey PIN, while the LUKS passphrase is passed only to =cryptsetup=.
