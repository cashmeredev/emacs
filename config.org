...
PATH (already provided through the Nix profile) and uses =vterm= as the
terminal backend (=eat= is also installed as a fallback).
...
** COMMENT org-modern (disabled in favor of render-org)
...
Nicolas Rougier's N Λ N O theme — companion to =nano-modeline=. Built around six base faces (default, critical, salient, faded, popout, subtle) and a deliberately restrained palette. Light variant uses Material colors, dark variant uses Nord.

We keep our own font (=MapleMono NF=) and skip nano's font setup (=nano-fonts-use=). =nano-window-divider-show= paints subtle dividers between windows so the minimal aesthetic still feels structured.
...
*  COMMENT org-modern-indent (disabled in favor of render-org)
