...
Native [[https://github.com/manzaltu/claude-code-ide.el][Claude Code IDE]] integration via MCP. Requires the =claude= CLI in
PATH (already provided through the Nix profile) and uses =eat= as the
terminal backend.
