...
A Rougier-style dashboard for the =croc= CLI file transfer tool (in =lisp/croc-ui.el=).  Sends files, folders and text, receives by code; each transfer is a real croc process with a live progress chip in the feed and its raw log one =RET= away.  The current share code gets a hero block and copies with =w=.
...
A Rougier-style dashboard for autosync/autopush of the local and redundant git repos (in =lisp/sync-ui.el=).  =s= runs a manual sync cycle: auto repos (org, skills, pass) commit everything dirty with a =host: ISO8601= message, rebase and push to every remote; ask repos (nix, emacs.d) offer a magit detour first and push on =r= once a commit exists.  A rebase conflict aborts and marks the repo CONFLICTED.  =a= / =d= / =t= add, remove and toggle repos, persisted via customize.
