...
(use-package ox-slidev
  ...
  :straight (:type git :host nil
             :repo "https://dev.cashmere.rs/cashmere/ox-slidev"
...
Requires tree-sitter grammars: =M-x treesit-install-language-grammar RET typescript= (and =tsx=).
...
(with-eval-after-load 'ewm
  ...
  (defun ewm-input--warp-pointer-to-window (window)
   ...
   ;; ── Modeline: move back to bottom, use minimal style ────
   (when (bound-and-true-p maple-modeline-mode)
     (setq maple-modeline-style 'ewm-style)
     (setq-default mode-line-format tab-line-format)
