...
To change the appearance of Emacs, lets start with the opacity. Setting it to 90% changes Emacs to a whole new level, as the colors of your theme and your background image harmonize with each otherd.
...
This is my own personal local leader configuration.
...
Here we configure the keymaps for keymaps in normal state.
