...
Per-host development overrides.  Any package registered in the gitignored =local.el= is loaded from a local checkout on =load-path= instead of its remote source, so I can hack on it in place (for example =kitty-graphics.el= on its =testing= branch) while =config.org= stays identical across every machine.

=local.el= is loaded inside =eval-and-compile= so the choice is consistent whether =config.el= runs as source or native-compiled.  After editing it, re-tangle/reload (or restart) so the native-compiled =config.el= picks up the new decision.  A minimal =local.el= looks like:

#+begin_example
;;; -*- lexical-binding: t -*-
(setq my/local-packages
      '((kitty-graphics . "~/code/kitty-graphics.el")))
#+end_example

The definitions live in =eval-and-compile= because =my/package-recipe= is called while a =:ensure= recipe is expanded, which happens at native-compile time as well as at load time.
...
(defun run-irc ()
  ...
  (let ((pw (my/soju-password)))
    ...
    (dolist (net '("libera" "ergo" "mansion"))
...
(use-package claude-code-ide
  ...
  (with-eval-after-load 'general
    ...
    (my-leader
      "aa" '(pi-coding-agent :wk "start")
      "at" '(pi-coding-agent-toggle :wk "toggle")
      "ao" '(pi-coding-agent-open-session-file :wk "open session")))
...
created later still get them."
  (when (display-graphic-p frame)
    ...
    (set-face-attribute 'fixed-pitch frame :family "IosevkaTerm NF" :weight 'regular)
    (set-face-attribute 'variable-pitch frame :family "IosevkaTerm NF" :weight 'regular :height 1.1)))
