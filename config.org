...
Only =y= (evil-yank) reaches the system clipboard; deletes (=d=, =x=, =c=) and word kills (M-DEL, C-w in insert) stay in the kill-ring only. Clipetty's global mode would sync every kill via OSC 52, so instead the OSC 52 escape is emitted explicitly after each yank. =elfeed-search-yank= (which bypasses evil-yank via =gui-set-selection=) gets the same treatment for TTY frames.
