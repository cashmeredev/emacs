...
** org-modern (disabled in favor of render-org)
...
*  org-modern-indent (disabled in favor of render-org)
...
Visual enhancement overlay for Emacs (cursor animation, window borders, etc).
Installed via Nix emacs package set — no elpaca needed.
