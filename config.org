...
Not on MELPA, so installed straight from GitHub. The tree-sitter grammar is built on first load via =treesit-install-language-grammar= (needs =cc= / =gcc= on PATH).
...
JavaScript and TypeScript support via tree-sitter modes.
Vue files use =web-mode=.
