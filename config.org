...
Per-host development overrides.  Any package registered in the gitignored =local.el= is loaded from a local checkout on =load-path= instead of its remote source, so I can hack on it in place (for example =kitty-graphics.el= on its =testing= branch) while =config.org= stays identical across every machine.  No per-package recipe wrapper is needed: an =:around= advice on the elpaca =:ensure= normalizer rewrites the recipe to =nil= for anything in =my/local-packages=, so the =use-package= forms downstream use the plain =:ensure (:host github :repo "...")= they always have and =local.el= overrides.

=local.el= is loaded inside =eval-and-compile= so the advice sees the same =my/local-packages= whether =config.el= runs as source or native-compiled.  After editing =local.el=, re-evaluate the file (or restart) so the new override takes effect.  A minimal =local.el= looks like:
...
(use-package agent-shell
  ...
  (setq agent-shell-anthropic-claude-environment
        (agent-shell-make-environment-variables
         ...
         "ANTHROPIC_AUTH_TOKEN" (password-store-get "minimax")))) 
#+end_src
...
Org Backend exporter for Slidev presentations.
...
=garden-player= (=M-x garden-player=, buffer =*navi*=) is a terminal Navidrome client: it streams through a background =mpv= over a unix socket, renders a TUI with =vui=, and shows album art inline through the kitty graphics protocol. It opens on your starred library; press =u= to flip the song list to the live up-next queue and =u= again to flip back.

The buffer installs =garden-player-map= as a vim-like keymap (bindings live in =garden-player.el=, this is just the reference):
...
| =h=   | previous track                                               |
| =l=   | next track                                                   |
| =j=   | restart the current song from the beginning                  |
| =k=   | cycle repeat mode: off → repeat-all → repeat-one             |
| =s=   | reshuffle the queue                                          |
| =n=   | search the library and play the pick next (no playlist made) |
| =N=   | search the library and append the pick to the end            |
| =/=   | same as =N=                                                    |
| =u=   | toggle between the starred library and the up-next queue     |
| =f=   | favorite the current song                                    |
| =r=   | radio: queue songs similar to the current one                |
| =g=   | reload the starred library                                   |
| =q=   | quit                                                         |
| =RET= | play the song on the current row                             |

Interactive commands: =garden-player= opens the player; =garden-player-play-next= and =garden-player-enqueue= search and queue without touching server playlists; =garden-player-toggle-view= flips the list; =garden-player-repeat=, =garden-player-shuffle=, =garden-player-radio=, =garden-player-like=, =garden-player-refresh= and =garden-player-quit= back the matching keys.
...
(use-package empv
  ...
  :defer t
  :config
  (setq empv-subsonic-url (password-store-get-field "navidrome" "url"))
  (setq empv-subsonic-username (password-store-get-field "navidrome" "username"))
  (setq empv-subsonic-password (password-store-get "navidrome"))

  (defun my/empv-subsonic-starred-songs ()
    "List every starred song in Navidrome and play the selected one."
    (interactive)
    (empv--subsonic-request
     "getStarred2.view"
     (empv--subsonic-result-handler "Starred song:")))

  (defun my/empv-subsonic-starred-songs-urls ()
    "Return a list of playable URLs for every starred song."
    (thread-last
      (empv--subsonic-request "getStarred2.view")
      (alist-get 'results)
      (seq-filter (lambda (item) (eq (alist-get 'kind item) 'song)))
      (mapcar #'empv--subsonic-item-extract-url)))

  (defun my/empv-subsonic-play-all-starred-songs ()
    "Clear the playlist and play every starred song."
    (interactive)
    (if-let ((urls (my/empv-subsonic-starred-songs-urls)))
        (empv-play urls)
      (message "No starred songs found")))

  (defun my/empv-subsonic-enqueue-all-starred-songs ()
    "Append every starred song to the current playlist."
    (interactive)
    (if-let ((urls (my/empv-subsonic-starred-songs-urls)))
        (empv-enqueue urls)
      (message "No starred songs found")))
