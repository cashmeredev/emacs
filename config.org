...
# Temporarily disabled while testing claude-code-ide — remove :tangle no to re-enable.
...
TUI-first beautiful rendering for org-mode (replaces org-modern).
...
Jupyter kernels in org-babel. =#+begin_src jupyter-python :session foo=
gibt persistente Python REPL, inline plots, async, completion.

Braucht system-side: =python3= mit =ipykernel jupyter-client pyzmq= +
=jupyter= CLI (siehe nix flake systems/md).

=emacs-zmq= wird beim ersten Laden native gebaut.
...
On Emacs 32 + a multi-TTY daemon (every ~Super+Return~ spawns =kitty emacsclient -nw -c=), the native tty child-frame popup tries to open a tty device for a closed client (~Could not open file: /dev/pts/N~) and spams the backtrace. =corfu-terminal= renders the popup as an overlay instead, so no frame and no device is ever opened.
...
=easysession-setup= restores the last session at startup and enables easysession-save-mode= for periodic auto-save. It is elpaca- and daemon-aware, so it picks =elpaca-after-init-hook= and per-frame restore on the daemon automatically.

mu4e- and ERC-buffers are process-backed and cannot be reconstructed from a file path, so easysession skips them. =my/easysession-restore-services= runs on =easysession-after-load-hook= and re-launches their entry points (=mu4e= in the background, =run-irc=) idempotently — guarded by =mu4e-running-p= and =erc-buffer-list= so switching between sessions never spawns a second instance or reconnects.
