...
(defcustom agent-shell-diff-delta-command
  '("delta" "--no-gitconfig" "--color-only")
  "Command and arguments used to colorize diffs with delta.

Set to nil to disable delta and fall back to the standard `diff-no-select'
output.  delta must be installed and on `exec-path'."
  :type '(choice (const :tag "Disable delta" nil)
                 (repeat string))
  :group 'agent-shell)
...
(defvar-local agent-shell-diff--used-delta nil
  "Non-nil when the current buffer's diff was generated using delta.")
        ...
        (progn
          (with-current-buffer diff-buffer
            (let ((inhibit-read-only t)
              ...
              (save-excursion
                (goto-char (point-min))
                ;; `diff-no-select' adds a command line at the top and
                ;; "Diff finished." at the bottom.  delta output does not,
                ;; so only strip those when delta was not used.
                (unless agent-shell-diff--used-delta
...
When `agent-shell-diff-delta-command' is non-nil and delta is available
on `exec-path', pipe the unified diff through delta for syntax
highlighting; otherwise fall back to `diff-no-select'."
  (let* ((suffix (format ".%s" (file-name-extension file)))
         (old-file (make-temp-file "old" nil suffix))
         (new-file (make-temp-file "new" nil suffix))
         (use-delta (and agent-shell-diff-delta-command
                         (executable-find (car agent-shell-diff-delta-command)))))
    (with-temp-file old-file (insert old))
    (with-temp-file new-file (insert new))
    (with-current-buffer buf
      (setq agent-shell-diff--used-delta use-delta)
      (if use-delta
          (let ((coding-system-for-read 'utf-8)
                (coding-system-for-write 'utf-8)
                (command (concat "diff -U3 "
                                 (shell-quote-argument old-file)
                                 " "
                                 (shell-quote-argument new-file)
                                 " | "
                                 (mapconcat #'shell-quote-argument
                                            agent-shell-diff-delta-command
                                            " "))))
            (call-process-shell-command command nil t nil)
            ;; `ansi-color-apply-on-region' can drop properties in some
            ;; Emacs builds, so apply to the string and re-insert.
            (let ((colored (ansi-color-apply (buffer-string))))
              (erase-buffer)
              (insert colored)))
