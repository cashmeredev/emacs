...
(defconst org-reveal-plugin-definitions
  '((markdown "dist/plugin/markdown.js" "RevealMarkdown")
    (highlight "dist/plugin/highlight.js" "RevealHighlight")
    (zoom "dist/plugin/zoom.js" "RevealZoom")
    (notes "dist/plugin/notes.js" "RevealNotes")
    (search "dist/plugin/search.js" "RevealSearch")
    (math "dist/plugin/math.js" "RevealMath")))
...
(defun org-reveal-stylesheets (info)
  (let ((root-path (file-name-as-directory (plist-get info :reveal-root))))
    (concat
     (format "
<link rel=\"stylesheet\" href=\"%sdist/reset.css\"/>
<link rel=\"stylesheet\" href=\"%sdist/reveal.css\"/>
<link rel=\"stylesheet\" href=\"%sdist/theme/%s.css\" id=\"theme\"/>
"
...
(defun org-reveal-plugin-spec (plugin)
  (cdr (assq plugin org-reveal-plugin-definitions)))

(defun org-reveal-plugin-list (info)
  (let ((configured (plist-get info :reveal-plugins)))
    (cond
     ((null configured) org-reveal-plugins)
     ((string= configured "") nil)
     (t (car (read-from-string configured))))))

(defun org-reveal-scripts (info)
  (let* ((root-path (file-name-as-directory (plist-get info :reveal-root)))
         (plugin-specs
          (delq nil
                (mapcar #'org-reveal-plugin-spec
                        (org-reveal-plugin-list info))))
         (extra-js (plist-get info :reveal-extra-js)))
    (concat
     (format "<script src=\"%sdist/reveal.js\"></script>\n" root-path)
     (mapconcat
      (lambda (spec)
        (format "<script src=\"%s%s\"></script>"
                root-path (nth 0 spec)))
      plugin-specs
      "\n")
     (if (string= extra-js "")
         ""
       (format "\n<script src=\"%s\"></script>" extra-js))
     "\n<script>\nReveal.initialize({\n"
     (format "controls: %s,\n"
     ...
     (if plugin-specs
         (format "plugins: [%s],\n"
                 (mapconcat (lambda (spec) (nth 1 spec))
                            plugin-specs
                            ", "))
       "")
     "});\n</script>\n")))
